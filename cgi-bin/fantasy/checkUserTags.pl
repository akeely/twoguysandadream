...
$sth = $dbh->prepare("SELECT t.player,t.type,t.locked,p.name,p.position FROM $tags_table t, players p WHERE t.team = '$team_t' AND t.league = '$league_t' and t.player=p.playerid and t.active='yes'")
